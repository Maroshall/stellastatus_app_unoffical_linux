...
> 📌 정식·베타 버전의 차이는 **[버전 안내](VERSIONING.md)** 를 참고하세요.
...
`v*` 태그를 push하면 GitHub Actions가 Linux AppImage를 빌드하고 GitHub Release에 자동으로 업로드합니다.
...
정식 버전은 `v1.0.7`, 베타 버전은 `v1.0.7.1`과 같이 네 자리 버전 태그를 사용합니다.
